...
if(photoFileInput) photoFileInput.addEventListener("change",()=>{
  ...
  if(!file){ if(preview) preview.hidden=true; if(img) img.removeAttribute("src"); return; }
  if(name) name.textContent=file.name;
  photoPreviewUrl=URL.createObjectURL(file);
  if(img){ img.style.display="block"; img.src=photoPreviewUrl; }
...
// Donation mobile-number copy control
const copyMobile=document.getElementById("copyMobile");
if(copyMobile) copyMobile.addEventListener("click",async()=>{
  try{await navigator.clipboard.writeText("8985120240");copyMobile.textContent="Copied!";setTimeout(()=>copyMobile.textContent="Copy",1500)}
  catch{copyMobile.textContent="Select & copy"}
});
